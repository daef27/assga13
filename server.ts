...
import { registrarArquivo, listarArquivos } from './src/db/users.ts';
...
function loadPersistedData() {
  try {
    ...
    if (parsed.parceiros && Array.isArray(parsed.parceiros)) {
      parceiros.splice(0, parceiros.length, ...parsed.parceiros);
    }
    if (parsed.mensagensContato && Array.isArray(parsed.mensagensContato)) {
      mensagensContato.splice(0, mensagensContato.length, ...parsed.mensagensContato);
    }
...
app.use((req: Request, res: Response, next) => {
  ...
  res.locals.parceiros = parceiros.filter(p => p.ativo).sort((a, b) => a.ordem - b.ordem);
  res.locals.totalParceiros = parceiros.length;
  res.locals.totalVoluntarios = voluntarios.length;
  res.locals.totalContatos = mensagensContato.length;
  next();
});
...
app.get('/contato', (_req: Request, res: Response) => {
  res.render('portal/contato', {
    title: 'Contato & Fale Conosco - ASSGA',
    activePage: 'contato',
  });
});

app.post('/contato', (req: Request, res: Response) => {
  const nome = asSingleString(req.body.nome).trim();
  const email = asSingleString(req.body.email).trim();
  const telefone = asSingleString(req.body.telefone).trim();
  const mensagem = asSingleString(req.body.mensagem).trim();

  if (!nome || !email || !telefone || !mensagem) {
    addFlash(req, 'Por favor, preencha todos os campos do formulário Fale Conosco.', 'danger');
    return res.redirect('/contato');
  }

  const novoContato: MensagemContato = {
    id: Date.now(),
    nome,
    email,
    telefone,
  ...
  mensagensContato.unshift(novoContato);
  persistDataStore();

  addFlash(req, 'Sua mensagem foi enviada com sucesso! A diretoria da ASSGA agradece o contato e retornará em breve.', 'success');
  res.redirect('/contato');
});

// Seja um Voluntário
app.get('/voluntario', (_req: Request, res: Response) => {
...
app.post('/voluntario', (req: Request, res: Response) => {
  ...
  const email = asSingleString(req.body.email).trim();
  const telefone = asSingleString(req.body.telefone).trim();
  const idade = asSingleString(req.body.idade).trim();
  const mensagem = asSingleString(req.body.mensagem).trim();

  if (!nome || !email || !telefone || !mensagem) {
    addFlash(req, 'Por favor, preencha todos os campos obrigatórios da inscrição de voluntário.', 'danger');
    return res.redirect('/voluntario');
  }
  ...
  const novoVoluntario: Voluntario = {
    ...
    email,
    telefone,
    idade: idade ? Number(idade) || idade : 'Não informada',
    mensagem,
    data_cadastro: new Date().toLocaleString('pt-BR'),
    status: 'Novo',
  };

  voluntarios.unshift(novoVoluntario);
  persistDataStore();

  addFlash(req, 'Cadastro realizado com sucesso! Você agora faz parte do movimento em apoio à comunidade surda da ASSGA. Em breve entraremos em contato!', 'success');
  res.redirect('/voluntario');
});

// Language Page / Acessibilidade & LIBRAS
app.get('/idioma', (_req: Request, res: Response) => {
  res.render('portal/idioma', {
    title: 'Acessibilidade Linguística & LIBRAS - ASSGA',
    activePage: 'idioma',
  });
});

app.get('/libras', (_req: Request, res: Response) => {
  res.redirect('/idioma');
});
...
app.post('/admin/associados/novo', upload.single('foto'), (req: Request, res: Response) => {
  ...
  if (req.file) {
    registrarArquivo({
      ...
      caminho: `/imagens/uploads/${req.file.filename}`,
      tamanho: req.file.size,
      tipoMime: req.file.mimetype,
    }).catch(err => console.warn('Erro ao registrar arquivo no SQL:', err));
  }
...
// 8. Gestão de Parceiros & Apoiadores no Rodapé
app.get('/admin/parceiros', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  res.render('admin/parceiros', {
    title: 'Gestão de Parceiros & Apoiadores - ASSGA',
    activePage: 'admin_parceiros',
    bodyClass: 'admin-theme',
    ...
    totalContatos: mensagensContato.length,
    parceiros,
  });
});

app.post('/admin/parceiros/novo', upload.single('logo'), (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const nome = asSingleString(req.body.nome).trim();
  const tipo = asSingleString(req.body.tipo).trim() as any;
  const link_url = asSingleString(req.body.link_url).trim();
  const descricao = asSingleString(req.body.descricao).trim();
  const ordem = Number(req.body.ordem) || (parceiros.length + 1);
  const ativo = req.body.ativo === 'true' || req.body.ativo === 'on' || req.body.ativo === true;
  const logo_url_externa = asSingleString(req.body.logo_url_externa).trim();

  let logo_url = '/imagens/parceiros/fpds.svg';
  if (req.file) {
    logo_url = `/imagens/uploads/${req.file.filename}`;
  } else if (logo_url_externa) {
    logo_url = logo_url_externa;
  }

  const novoParceiro: Parceiro = {
    id: Date.now(),
    nome: nome || 'Novo Parceiro Oficial',
    tipo: tipo || 'Apoiador Esportivo',
    logo_url,
  ...
  addFlash(req, `Parceiro "${novoParceiro.nome}" cadastrado com sucesso para o rodapé!`, 'success');
  res.redirect('/admin/parceiros');
});

app.post('/admin/parceiros/:id/toggle', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const id = Number(req.params.id);
  const p = parceiros.find(item => item.id === id);
  if (p) {
    p.ativo = !p.ativo;
    persistDataStore();
    addFlash(req, `Exibição do parceiro "${p.nome}" no rodapé foi ${p.ativo ? 'ativada' : 'desativada'}.`, 'info');
  }

  res.redirect('/admin/parceiros');
});

app.post('/admin/parceiros/:id/excluir', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const id = Number(req.params.id);
  const index = parceiros.findIndex(item => item.id === id);
  if (index !== -1) {
    const nome = parceiros[index].nome;
    parceiros.splice(index, 1);
    persistDataStore();
    addFlash(req, `Parceiro "${nome}" removido com sucesso.`, 'warning');
  }

  res.redirect('/admin/parceiros');
});

// 9. Gestão de Inscrições de Voluntários
app.get('/admin/voluntarios', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  res.render('admin/voluntarios', {
    title: 'Inscrições de Voluntários - ASSGA',
    activePage: 'admin_voluntarios',
    bodyClass: 'admin-theme',
...
app.post('/admin/voluntarios/:id/status', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const id = Number(req.params.id);
  const novoStatus = asSingleString(req.body.status).trim() as any;
  const v = voluntarios.find(item => item.id === id);
  if (v && novoStatus) {
    v.status = novoStatus;
    persistDataStore();
    addFlash(req, `Status do voluntário "${v.nome}" atualizado para "${novoStatus}".`, 'success');
  }
...
app.post('/admin/voluntarios/:id/excluir', (req: Request, res: Response) => {
  ...
  if (index !== -1) {
    ...
    persistDataStore();
    addFlash(req, 'Cadastro de voluntário excluído.', 'warning');
  }

  res.redirect('/admin/voluntarios');
});

// 10. Gestão de Mensagens do Fale Conosco
app.get('/admin/contatos', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  res.render('admin/contatos', {
    title: 'Mensagens Fale Conosco - ASSGA',
    activePage: 'admin_contatos',
    bodyClass: 'admin-theme',
    activeAdminTab: 'contatos',
    totalAssociados: associados.length,
    totalMensalidades: mensalidades.length,
    totalCarteirinhas: carteirinhas.length,
    totalEventos: eventos.length,
...
app.post('/admin/contatos/:id/status', (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const id = Number(req.params.id);
  const novoStatus = asSingleString(req.body.status).trim() as any;
  const m = mensagensContato.find(item => item.id === id);
  if (m && novoStatus) {
    m.status = novoStatus;
    persistDataStore();
    addFlash(req, `Status da mensagem atualizado para "${novoStatus}".`, 'info');
  }
...
app.post('/admin/contatos/:id/excluir', (req: Request, res: Response) => {
  ...
  const id = Number(req.params.id);
  const index = mensagensContato.findIndex(item => item.id === id);
  if (index !== -1) {
    mensagensContato.splice(index, 1);
    persistDataStore();
    addFlash(req, 'Mensagem excluída com sucesso.', 'warning');
  }

  res.redirect('/admin/contatos');
});

// 12b. Arquivos & Uploads no Banco SQL
app.get('/admin/arquivos', async (req: Request, res: Response) => {
  if (!requireAdmin(req, res)) return;

  const arquivosList = await listarArquivos();

  res.render('admin/arquivos', {
    title: 'Arquivos & Uploads no Banco SQL - ASSGA',
    activePage: 'admin_arquivos',
    bodyClass: 'admin-theme',
    activeAdminTab: 'arquivos',
    totalAssociados: associados.length,
    totalMensalidades: mensalidades.length,
    totalCarteirinhas: carteirinhas.length,
    totalEventos: eventos.length,
...
app.get('/api/arquivos', async (_req: Request, res: Response) => {
  try {
    const arquivosList = await listarArquivos();
    res.json({ success: true, total: arquivosList.length, data: arquivosList });
  } catch (error: any) {
    res.status(500).json({ success: false, error: error.message || 'Erro ao listar arquivos' });
  }
});
// 13. Health API
